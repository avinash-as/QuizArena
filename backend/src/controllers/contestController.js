...
// other than DRAFT) without at least one question — otherwise it could go
// LIVE with nothing for players to answer.
const assertContestReadyToPublish = async (status, quizId, prizePool, prizeBreakdown) => {
  ...
  // A contest with a real prize pool but an empty prizeBreakdown silently
  // pays out nobody when it completes — distributePrizes() just iterates
  // zero prize tiers and returns. This exact bug happened in production
  // (Quiz #1: prizePool 1000, prizeBreakdown []) before this check existed.
  if (prizePool > 0 && (!prizeBreakdown || prizeBreakdown.length === 0)) {
    const err = new Error('This contest has a prize pool but no prize tiers set. Add at least one rank (e.g. 1st Place) before publishing, or winners will not get paid.')
    err.statusCode = 400
    throw err
  }
}
...
exports.createContest = async (req, res, next) => {
  try {
    ...
    if (body.status && body.status !== 'DRAFT') {
      await assertContestReadyToPublish(body.status, createdQuiz._id, body.prizePool, body.prizeBreakdown)
    }
...
exports.updateContest = async (req, res, next) => {
  try {
    const existing = await Contest.findById(req.params.id).select('quiz status prizePool prizeBreakdown')
    if (!existing) return res.status(404).json({ success: false, message: 'Contest not found' })
    ...
    const nextStatus         = body.status ?? existing.status
    const nextPrizePool      = body.prizePool ?? existing.prizePool
    const nextPrizeBreakdown = body.prizeBreakdown ?? existing.prizeBreakdown
    await assertContestReadyToPublish(nextStatus, existing.quiz, nextPrizePool, nextPrizeBreakdown)
